...
#
#   bash <(curl -sL "http://81.68.84.205:5244/dav/TY/flash_tool/install.sh")
#
# 或先下载再执行：
#   curl -sLo ~/install.sh "http://81.68.84.205:5244/dav/TY/flash_tool/install.sh" && bash ~/install.sh
#
set -e
...
OFFLINE_DEP_URL="http://81.68.84.205:5244/sd/123456/offline_deps.tar.gz"
...
do_deploy() {
  ...
  info "安装 Termux 依赖包..."
  local need_pkgs=""
  for pkg in python python-pip termux-api android-tools zstd p7zip unrar unzip libusb; do
  ...
  yes "" | python -m pip install flask flask-socketio simple-websocket --break-system-packages --ignore-installed blinker -i "$PIP_MIRROR_URL" || {
