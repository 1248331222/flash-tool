...
split_args / strip_quote 来自 core.bat_parser（参数切分公共辅助），延迟导入避免循环。
...
# 延迟导入 split_args/strip_quote（避免 bat_parser <-> fastboot_cmd_parser 循环导入）
def _get_split_args():
    from core.bat_parser import split_args
    return split_args

def _get_strip_quote():
    from core.bat_parser import strip_quote
    return strip_quote
