...
def _read_and_parse_script(filepath, extra_args=""):
    """
    读取并解析刷机脚本（公共逻辑）
    使用天树引擎解析。
    ...
    Args:
        filepath: 脚本文件的完整路径
        extra_args: 用户传入的额外全局参数（对应脚本中 %* 占位符）
    ...
    if is_sh:
        ...
        result = hydra.parse(txt, script_type="bat", rom_dir=rom_dir, script_path=filepath, extra_args=extra_args)

    if result.total_steps > 0 or (result.class_id == "interactive" and result.pending_choices):
...
def _hydra_steps_to_old(hydra_steps):
    ...
    for hs in hydra_steps:
        ...
        # 优先使用已提取的 params，否则从 raw 中提取 prefixParams
        if hs.params and hs.params.strip().startswith('--'):
            step["prefixParams"] = hs.params.strip()
        elif prefix_params:
