...
class SymbolTable:
    ...
    def _resolve_bat(self, text: str, line_no: int = 0) -> str:
        ...
        # 0. 展开 BAT 位置参数 %1-%9 及修饰符 %~1, %~dp1, %~nx1 等
        def _repl_positional(m):
            vname_raw = m.group(1)  # 如 ~1, ~dp1, 1
            # 提取修饰符前缀
            modifier = ''
            base_num = vname_raw
            for mod in ('~dp', '~nx', '~n', '~x', '~f', '~p', '~'):
                if vname_raw.startswith(mod):
                    modifier = mod
                    base_num = vname_raw[len(mod):]
                    break
            resolved = self._resolve_ref(base_num, line_no)
            if modifier:
                if modifier == '~':
                    return resolved.strip('"')
                elif modifier == '~dp':
                    return os.path.dirname(os.path.abspath(resolved)) + os.sep if resolved else ''
                elif modifier == '~nx':
                    return os.path.basename(resolved) if resolved else ''
                elif modifier == '~n':
                    return os.path.splitext(os.path.basename(resolved))[0] if resolved else ''
                elif modifier == '~x':
                    return os.path.splitext(resolved)[1] if resolved else ''
                elif modifier == '~f':
        ...
        result = re.sub(r'%((?:~dp|~nx|~n|~x|~f|~p|~)?[1-9])', _repl_positional, text)
