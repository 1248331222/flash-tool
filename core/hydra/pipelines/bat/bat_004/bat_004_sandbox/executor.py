...
class BatSandbox:
    ...
    def __init__(self, device_info: Optional[Dict] = None, rom_dir: str = '', extra_args: str = ''):
    ...
    def _run_branches(self, cmds: List[NormalizedCmd]) -> List[dict]:
        """分支模拟模式：对所有交互变量做笛卡尔积穷举执行
        
        支持两种脚本结构：
        1. 菜单式：一个 label 统一下发，只对第一个交互变量分支化
        2. 线性式：多个交互变量顺序出现，需笛卡尔积穷举所有组合
        """
        import itertools
        ...
        for cmd in cmds:
            if cmd.type == 'INTERACTIVE':
                vn = cmd.args[0].lower()
                if vn in self.interactive_vars and vn not in var_names:
                    var_names.append(vn)
        
        if not var_names:
            return self._run_single(cmds)
        
        # 扫描第一个交互入口 label（作为起始位置）
        start_pc = 0
        ...
        var_values = []
        for vn in var_names:
            info = self.interactive_vars.get(vn, {})
            branches = sorted(info.get('branches', {''}))
            if not branches:
                branches = ['']
            var_values.append(branches)
        
        all_results = []
        
        for combo in itertools.product(*var_values):
            # 构建组合描述
            choice_parts = [f'{vn}={val}' for vn, val in zip(var_names, combo)]
            ...
            for vn, val in zip(var_names, combo):
                sandbox.env[vn] = str(val)
            sandbox._delayed_expansion = self._delayed_expansion
            
            steps = sandbox._run_single(cmds, max_gotos=20, start_pc=start_pc)
            
            if steps:  # 只记录有产出的组合
                all_results.append({
                    'choice': choice_str,
                    'var_name': var_names[0],
                    'branch_value': combo[0],
                    'prompt': self.interactive_vars.get(var_names[0], {}).get('prompt', ''),
                    'steps': steps,
