...
编排四阶段流程：
  0 → ShPreScanner.scan(content)
  1 → ShSandboxRunner.run(script_path, rom_dir)
  2 → ShDecisionExtractor.extract(jsonl)
  3 → 等待用户决议（由前端处理）
  4a → ShCommandExpander.expand(jsonl, resolutions)
  4b → ShCommandReader.parse(commands)
  4c → ShRiskAnalyzer.analyze(steps, rom_dir, mode)
  4d → ShBlockBuilder.build(steps)
"""
...
class ShEngine:
    ...
    用法:
        engine = ShEngine(mode=ShParseMode.FULL)
        result = engine.parse(content, script_path, rom_dir)
        if result.pending_decisions:
            # 展示给用户选择
            resolutions = {"slot": "_a"}
            result = engine.resolve(result, resolutions)
    """
