...
class BatNormalizer:
    ...
    IGNORE_PATTERNS = [
        r'^\s*@?(echo|rem|::|cls|pause|title|color|chcp|cd\b)',
        r'^\s*@?echo\b',
        r'^\s*$',
        r'^\s*@?\s*$',
        r'^\s*\)',           # 闭合括号单独一行
    ...
    def _is_block_start(self, line: str) -> bool:
        ...
        # 必须行尾是 ) 且行中有 (（排除 in (X) 类型的单行括号）
        if not stripped.rstrip().endswith('('):
            return False
        
        # 如果是 for ... in (...) do (，括号数 > 1 或 do ( 在行尾 => 是多行块
        # for ... in (...) 单行时括号成对出现，且行尾无显式 (
        # 
        # 检查行尾的 ( 是否是独立的块开始括号
        # 简单规则：行尾 ( 前不是 do 或 ( 则可能是多行块
        
        # for %%p in (%PARTITIONS%) do ( → 块开始，因为 do ( 在行尾
        # for %%p in (a b c) do fastboot → 单行，不是块开始
        # if exist "X" ( → 块开始
