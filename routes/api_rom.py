...
def list_extracted_roms():
    ...
    try:
        ...
        for d in sorted(dirs):
            ...
            dir_list.append({"name": d, "type": rom_type})
...
def import_bat_file():
    ...
    try:
        ...
        txt, steps, is_complex, is_native_sh, complex_reason, missing_files = result
        # 根据解析结果确定脚本类型：SH 脚本如果可解析（非原生），标记为 bat 类型
        lower_path = bat_path.lower()
        if lower_path.endswith('.sh'):
            script_type = 'sh' if is_native_sh else 'bat'
        elif is_complex:
            script_type = 'bat_complex'
        else:
            script_type = 'bat'
        resp = {
            "success": True,
            ...
            "is_native_sh": is_native_sh,
            "script_type": script_type,
            "complex_reason": complex_reason or "",
