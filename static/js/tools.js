...
async function checkUpdate(silent) {
    ...
    try {
        ...
        const d = await parseApiResponse(await fetch((window.BACKEND_API_URL || '') + '/api/update/check', {signal: ctrl.signal}), '/api/update/check');
        clearTimeout(timer);
        if (!d.success) {
            if (silent) return; // 静默模式下检查失败不提示
        ...
        if (silent) return; // 静默模式下任何错误都不提示
