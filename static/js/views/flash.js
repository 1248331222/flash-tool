...
async function startBatchFlash() {
    ...
    const selectedRomProject = document.getElementById('romProjectSelect').value;
    ...
    if (saved) {
        try {
            ...
            const romMatch = data.rom_name === selectedRomProject;
            if (romMatch && data.steps.length === stepList.length) {
...
Modules.register('flash', ['api','utils','bat','bat-risk'], function initFlashModule() {
    document.getElementById('batchFlashBtn').onclick = startBatchFlash;

    console.log('[flash] 线刷模块已初始化');
    return true;
});
