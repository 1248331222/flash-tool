...
Modules.register('bat', ['api','utils','rom'], function initBatModule() {
    ...
    document.getElementById('importBatBtn').onclick = async () => {
        ...
        try {
            const extra_args = document.getElementById('batExtraArgs').value || '';
            const d = await apiPost('/api/rom/import_bat', {rom_name: rn, bat_path: bp, extra_args: extra_args});
            if (d.success) {
                stepList = d.steps || [];
                window._shContent = '';
