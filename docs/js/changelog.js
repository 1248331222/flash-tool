...
v3.3.1 (2026-06-30)
- Hydra 引擎：修复 BAT if 条件变量展开判等逻辑（乱判断镜像类型）
- Hydra 引擎：修复 BAT 通配符 for 体内 call :label 内联展开后再解析的步骤丢失
- Hydra 引擎：新增 test_hydra.py 单元测试框架（32项测试用例，12个样本脚本）
- Hydra 引擎：修复 SH 循环体内 simple_flag 未重置，后续普通命令被误判为循环体
