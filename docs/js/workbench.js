...
function wbSwitchType(type) {
    ...
    document.getElementById('wbAddShell').style.display = type === 'shell' ? '' : 'none';
}
...
// 添加 Shell 步骤
function wbAddShellStep() {
    const cmd = document.getElementById('wbShellCmd').value.trim();
    if (!cmd) return writeLog('请输入Shell命令', 'err');
    wbSteps.push({id: Date.now(), type: 'shell', command: '', args: cmd, raw: cmd, desc: wbDescribe(cmd), status: 'pending', enabled: true});
    renderWbSteps();
    document.getElementById('wbShellCmd').value = '';
    writeLog(`工作台：${wbDescribe(cmd)}`, 'info');
}
